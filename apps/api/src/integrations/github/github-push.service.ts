import { randomUUID } from 'node:crypto';
import { Injectable } from '@nestjs/common';
import { PrismaService } from '../../persistence/prisma.service';
import { WorkspaceService } from '../../generation-engine/workspace.service';
import { ProcessRunnerService } from '../../generation-engine/process-runner.service';
import { GithubCredentialService } from './github-credential.service';

const GITHUB_API = 'https://api.github.com';

export interface GithubPushDto {
  missionId: string;
  status: 'PENDING' | 'PUSHED' | 'FAILED';
  repoName: string | null;
  repoUrl: string | null;
  errorCode: string | null;
  logsExcerpt: string | null;
}

/**
 * MISSÃO "GitHub real (push de verdade)" — cria o repositório de verdade via API do GitHub e
 * empurra o workspace gerado com `git` real (mesmo ProcessRunnerService já usado para
 * build/test/runtime — nenhum caminho paralelo inventado). Nunca envia código que não passou nos
 * gates reais de entrega (deliveryEligible + securityPassed) — mesma exigência do download.
 */
@Injectable()
export class GithubPushService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly workspace: WorkspaceService,
    private readonly runner: ProcessRunnerService,
    private readonly credentials: GithubCredentialService
  ) {}

  async getStatus(missionId: string): Promise<GithubPushDto | null> {
    const row = await this.prisma.githubPush.findUnique({ where: { missionId } });
    return row ? this.toDto(row) : null;
  }

  async push(missionId: string, repoName: string, isPrivate: boolean): Promise<GithubPushDto> {
    const run = await this.prisma.missionGenerationRun.findUnique({ where: { missionId } });
    if (!run || run.status !== 'READY' || !run.downloadPath) throw new Error('GENERATION_NOT_READY');
    if (!run.deliveryEligible) throw new Error('DELIVERY_NOT_ELIGIBLE');
    if (!run.securityPassed) throw new Error('SECURITY_GATE_FAILED');

    const { token, login } = await this.credentials.getDecryptedToken();
    const workspacePath = this.workspace.workspacePathFor(missionId);

    await this.prisma.githubPush.upsert({
      where: { missionId },
      create: { id: randomUUID(), missionId, status: 'PENDING', repoName },
      update: { status: 'PENDING', repoName, errorCode: null, repoUrl: null, logsExcerpt: null },
    });

    try {
      const repoUrl = await this.createRepo(token, repoName, isPrivate);
      const logs = await this.pushWorkspace(workspacePath, token, login, repoName);
      const updated = await this.prisma.githubPush.update({
        where: { missionId },
        data: { status: 'PUSHED', repoUrl, logsExcerpt: this.scrubToken(logs, token) },
      });
      return this.toDto(updated);
    } catch (err) {
      const errorCode = err instanceof Error ? err.message : 'GITHUB_PUSH_FAILED';
      const updated = await this.prisma.githubPush.update({
        where: { missionId },
        data: { status: 'FAILED', errorCode, logsExcerpt: this.scrubToken(err instanceof Error ? err.message : '', token) },
      });
      return this.toDto(updated);
    }
  }

  private async createRepo(token: string, repoName: string, isPrivate: boolean): Promise<string> {
    let response: Response;
    try {
      response = await fetch(`${GITHUB_API}/user/repos`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'User-Agent': 'ldcn-os', 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: repoName, private: isPrivate, auto_init: false }),
      });
    } catch {
      throw new Error('GITHUB_UNAVAILABLE');
    }
    if (response.status === 422) throw new Error('GITHUB_REPO_NAME_TAKEN');
    if (!response.ok) throw new Error('GITHUB_REPO_CREATE_FAILED');
    const body = (await response.json()) as { html_url?: string };
    if (!body.html_url) throw new Error('GITHUB_REPO_CREATE_FAILED');
    return body.html_url;
  }

  /** git real via ProcessRunnerService — o mesmo runner que já executa npm install/build/test,
   * nenhum caminho paralelo. Nunca inclui o token no remote sem escaneá-lo de volta antes de
   * persistir qualquer log (scrubToken). */
  private async pushWorkspace(workspacePath: string, token: string, login: string, repoName: string): Promise<string> {
    const remoteUrl = `https://x-access-token:${token}@github.com/${login}/${repoName}.git`;
    const isRepo = await this.workspace.pathExists(`${workspacePath}/.git`);
    const steps: { command: string; args: string[] }[] = [
      ...(isRepo ? [] : [{ command: 'git', args: ['init'] }]),
      { command: 'git', args: ['add', '-A'] },
      { command: 'git', args: ['-c', 'user.email=ldcn-os@local', '-c', 'user.name=LDCN OS', 'commit', '-m', 'Geração real via LDCN OS', '--allow-empty'] },
      { command: 'git', args: ['branch', '-M', 'main'] },
      { command: 'git', args: ['remote', 'remove', 'origin'] }, // ignora falha se não existir ainda
      { command: 'git', args: ['remote', 'add', 'origin', remoteUrl] },
      { command: 'git', args: ['push', '-u', 'origin', 'main', '--force'] },
    ];

    let combinedLogs = '';
    for (const step of steps) {
      const result = await this.runner.runCommand(step.command, step.args, workspacePath, 60_000);
      combinedLogs += `$ ${step.command} ${step.args.filter((a) => a !== remoteUrl).join(' ')}\n${this.scrubToken(result.logsExcerpt, token)}\n\n`;
      // "remote remove origin" pode falhar legitimamente na 1ª vez (origin ainda não existe) —
      // nunca trata isso como erro real do push.
      const isRemoveOrigin = step.args[0] === 'remote' && step.args[1] === 'remove';
      if (result.exitCode !== 0 && !isRemoveOrigin) {
        throw new Error(`GITHUB_PUSH_COMMAND_FAILED: ${step.command} ${step.args[0]}`);
      }
    }
    return combinedLogs;
  }

  private scrubToken(text: string, token: string): string {
    return text.split(token).join('***');
  }

  private toDto(row: { missionId: string; status: string; repoName: string | null; repoUrl: string | null; errorCode: string | null; logsExcerpt: string | null }): GithubPushDto {
    return {
      missionId: row.missionId, status: row.status as GithubPushDto['status'],
      repoName: row.repoName, repoUrl: row.repoUrl, errorCode: row.errorCode, logsExcerpt: row.logsExcerpt,
    };
  }
}
